...
const sendemergencyCntrl = asyncHandler(async (req, res) => {
 ...
 // const { userId, lat, long } = req.body;
  ...
  const resp = await getData(
   ` https://apis.mapmyindia.com/advancedmaps/v1/efd1bc9e76b7a36cb990af517a48f3c3/rev_geocode?lat=${lat}&lng=${long}`
  );
  ...
  // ✅ FIX 3: removed blank push()
  const recipients = [user.emergencyMail];

  // ✅ FIX 4: email logic corrected
  if (user.extraEmail1) {
